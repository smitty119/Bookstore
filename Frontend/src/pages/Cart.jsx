...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get("http://localhost:3002/cart/usercart", { headers });
  ...
  const deleteItem = async (bookid) => {
    const response = await axios.put(`http://localhost:3002/cart/removefromcart/${bookid}`, {}, { headers });
  ...
  const placeOrder = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3002/order/placeorder",
        { order: Cart },
        { headers }
      );
      alert(response.data.message);
